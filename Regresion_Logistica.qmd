...
b. Precision de 0.74 es moderado. Describe que la mayoria de predicciones estan cercanas al real. Analogo, al accuracy pero el valor para este caso es bajo.
...
f. Curva ROC de 0.7986 significa que sí tomamos al azar un nacimiento que resultó en la clase cesárea y un nacimiento que resultó normal, el modelo tiene una probabilidad del 79.86% de asignar una probabilidad más alta al nacimiento que realmente pertenece a cesárea.

![image.png](data/output4.png)

El comportamiento de estas metricas y su valor moderado se atribuye a la correlación de las caracteristicas con la variable Target. En el EDA no existe una correlación suficientemente fuerte.

Para mejorar estas metricas, se recomendaria realizar una ingenieria de caracteristicas sobre el dataset. Y generar o incorporara al dataset caracteristicas que aporten mas información en la etapa de entrenamiento. 

Ademas, la explicación de que la mayoria de metricas esten cerca del 0.73 se explica por el balance de la variable objetivo.
